...
| Member aliases                  | scripts/prod/migrations/055_member_aliases.sql  |
...
- **🏷️ Aliases are the display name.** `members.alias` (optional, 2–20 chars, letters/digits/spaces, unique case-insensitively — migration 055) is the short name the batch uses. **Never render `member.name` directly** — call `memberDisplayName(member)` from `@/lib/member-alias`, which falls back to the full name when there's no alias. Member *pickers* use `memberSelectOptions(members)` + `filterBy={MEMBER_FILTER_BY}` so the label shows "Alias · Full Name" and typing either half matches. Read functions that shape rows for the UI (`dashboard.ts`, `polls.ts`, `loans.ts`, …) do the swap once at the source: `member_name` carries the *display* name and `member_full_name` the canonical one. Writes go through `fn_set_member_alias` / `fn_set_member_aliases` (SECURITY DEFINER) — `members` is admin-write under RLS, but a member may change their own alias.
